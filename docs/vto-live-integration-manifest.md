...
| `docs/vto-live-productization-v1.md` | The productization & runtime-readiness lane's own source-of-truth record: the tracking-quality contract and its frozen thresholds, the capture authority, the truthful capability decision, the garment lifecycle, the staging activation state and its open owner action, the carried holds, and the four decision memos. Required by that lane's own §60 (update the VTO developer/source-of-truth documentation). Declared as an EXACT path, never by widening `docs/vto-*`: the guard refused it, which is the guard working. | VTO Productization & Runtime-Readiness lane §60 (2026-09-07) |
| `docs/vto-physical-qa-run-packet.md` | The mandatory manual certification packet: six numbered device journeys, the evidence drop format, the perceptual quality matrix, the holds a tester must record rather than debug, and the "Live never appeared" diagnosis table. Required as a deliverable by that lane's own §43-§45, and the artifact the later physical-device session is run from. Declared as an EXACT path for the same reason as the row above. | VTO Productization & Runtime-Readiness lane §43, §44, §45 (2026-09-07) |
...
| `hooks/useVtoMode.ts` | VTO V2's single React binding for the mode authority -- the ONE hook a customer surface asks. Declared as an exact path rather than by widening the `hooks/useVto*` prefix (`useVtoAvailability*` and `useVtoLive*` are the existing exact grants, and a wildcard would authorize a hook nobody has reviewed). It gathers already-resolved evidence and decides nothing: every verdict comes back from `services/vto/vtoModeAuthority.ts`. | VTO V2 lane §8, §24, §25; `docs/vto-v2-customer-activation.md` |
| `docs/vto-live-asset-intake.md` | VTO V2's required rights-cleared Live asset intake specification: source rights, allowed source types, required view, background/alpha/format/dimensions/colour space, category and product mapping, the asset factory command, the generated descriptor's accept criteria, both platform outputs, manifest and registry entries, the parity test, and the visual review step. Declared as an EXACT path, never by widening `docs/vto-*`, matching the `docs/vto-live-productization-v1.md` precedent. | VTO V2 lane §13, §44 |
| `docs/vto-v2-customer-activation.md` | VTO V2's own source-of-truth lane record: the entry defect and its measured before/after, the single mode authority, the declared runtime-proof status for both modes, the Photoreal provider evidence and the owner paid-smoke-test recommendation, the coverage and reason distributions, the customer flow, the result-to-Commerce loop, the privacy/data boundary, the telemetry funnel and the future binding metric definitions. Declared as an EXACT path for the same reason as the row above. | VTO V2 lane §60 |
| `supabase/functions/vto-generate/vtoHandler.ts` | VTO V3.1: the governed failure exit collapsed three different truths into one code. `reservation_quota` (the actor's real allowance), `reservation_duplicate` (a request already running) and a vendor gateway 429 all returned `rate_limited`, which the app rendered as "You've reached the try-on limit for now" -- false for two of the three. The repair splits them into `quota_exhausted` / `request_in_flight` / `provider_busy` and adds ONE bounded integer (`retryAfterSeconds`) to the error envelope. `stage` and `providerDetail` stay server-log only. Declared as an EXACT path rather than by widening to `supabase/functions/vto-generate/**`: the guard refused it, which is the guard working. | VTO V3.1 mission §2, §3, §4 (customer-facing 429 truth) |
| `supabase/functions/vto-generate/vtoContract.ts` | VTO V3.1: three additive failure codes and the `Retry-After` bounds (`VTO_RETRY_AFTER_MIN/MAX_SECONDS`), plus `retryAfterSeconds` on the provider-outcome type. Additive only -- no existing code was removed, and `types/vto.ts` is kept in agreement by `__tests__/vtoContractParity.test.js`. Exact path, not a directory grant. | VTO V3.1 mission §2, §5 |
| `supabase/functions/vto-generate/providers/aiLabToolsProvider.ts` | VTO V3.1: an HTTP 429 from the vendor gateway is reclassified from `rate_limited` to `provider_busy` (K Scan is being throttled, not the customer), and `Retry-After` -- previously never read at all -- is parsed into bounded whole seconds by the new pure `parseRetryAfterSeconds`. `billable` semantics are deliberately unchanged, so quota accounting is untouched. Exact path, not a directory grant. | VTO V3.1 mission §2, §5, §7 |
| `supabase/functions/vto-generate/providers/aiLabToolsProvider.test.ts` | Regression proof for the row above: the assertion that used to read `429 maps to rate_limited` was the defect written down, and now pins `provider_busy` plus the `Retry-After` parse across both RFC 9110 forms and every hostile/out-of-range input. Exact path; does not authorize edge-function tests generally. | VTO V3.1 mission §8 (BLOCK-VTO31-02..05) |
