...
## N1-ENV-012 (P0, native renderer) — the renderer never actually painted, on any device, since N1-B's inception

**Found:** N1-E, by the mandatory physical-device screenshot (amendment D2) -- the first time in this lane's history that a real screenshot of the rendered View was actually inspected pixel-by-pixel rather than accepted on the strength of the geometry log. Physical device: Samsung SM-S936U, Android 16 (API 36), arm64-v8a.

**Reproduction:** deep-link to `kscan://dev-n1-diagnostic` with an authenticated session, capture the screen. The `N1-B static render` box and the `N1-D native replay` box both showed only their `styles.container` parent's background color (`#101014`) -- not the Kotlin view's own `Color.rgb(32,32,36)` background paint, not the yellow landmark markers, not the garment mesh, not a red error string. All three prior on-device geometry logs (N1-A capability, N1-B snapshot, N1-D replay-to-EOF) were simultaneously CORRECT, proving the bug was specific to painting, not to geometry.

**Root cause, confirmed by direct instrumentation (not inferred):** added a one-line `Log.d` at the top of `onDraw` reporting view width/height/bitmap/vertex count. It never fired -- not once, across repeated deep-links, meaning `onDraw` was never being INVOKED by the Android View system at all. `ExpoView` (the Expo Modules base class every native view in this codebase extends) itself extends `LinearLayout`, a `ViewGroup`. `ViewGroup`'s own constructor calls `setWillNotDraw(true)` by default, on the standard assumption that a group draws nothing of its own and only composites its children's content. `LiveVtoTestRenderView` overrides `onDraw(Canvas)` to paint directly onto the group itself, but never called `setWillNotDraw(false)` to tell the View system to expect that -- so the entire override was silently skipped by an Android platform optimization, on every device and every emulator, since N1-B was first written.

**An earlier, wrong hypothesis, corrected rather than left uncorrected:** the first attempt at a root cause guessed `Canvas.drawBitmapMesh` specifically was unsupported under hardware acceleration (a real, documented historical Android limitation) and applied `setLayerType(LAYER_TYPE_SOFTWARE, null)` as the fix. That did not resolve the blank render, because the true defect was one layer up: `onDraw` was never called in the first place, so `drawBitmapMesh` was never reached regardless of layer type. The `LAYER_TYPE_SOFTWARE` call was kept anyway (cheap insurance against a real, separate class of GPU-driver risk for this diagnostic surface) but is not the fix; `setWillNotDraw(false)` is.

**Why nothing else ever caught this:**
- Numerical geometry conformance (`LiveVtoGeometryPipeline`, amendment D8) measures the same pipeline this view draws FROM, computed BEFORE rasterization by design -- it was never wired to observe whether the View subsequently painted anything, and correctly reported PASS throughout, because the geometry really was correct throughout.
- The off-device rasterizer (`tools/render-snapshot.mjs`) reimplements the mesh math in Node to produce human-reviewable PNGs. It is a SEPARATE implementation of the same formulas, not a capture of the real `Canvas.drawBitmapMesh` call, and could never have caught a defect in the real Android draw invocation.
- Every emulator screenshot attempted in N1-B/N1-C/N1-D was blocked by the (correctly-behaving) auth routing guard before a render was ever visible, so this session is the first one in which a rendered frame was actually inspected at all.

This is the concrete argument amendment D2 makes for requiring a real screenshot rather than accepting numerical conformance as sufficient: it is possible for every number to be right and the screen to show nothing.

**Repair:** `LiveVtoTestRenderView.init` now calls `setWillNotDraw(false)` (the actual fix) and `setLayerType(View.LAYER_TYPE_SOFTWARE, null)` (retained defense-in-depth, not the fix).

**Regression:** rebuilt, reinstalled on the physical device, re-tested. `onDraw` now fires with correct data (`viewW=1012 viewH=844 bitmap=271x302 bitmapConfig=ARGB_8888 verts=160 meshW=7 meshH=9`), and the physical-device screenshot shows a correctly-shaped garment silhouette with visible yellow landmark markers at the expected shoulder/hip positions -- `evidence/vto-live-native-n1/n1b-physical-device-screenshot.png` (full) and `n1b-physical-device-render-closeup.png` (cropped). The N1-D replay box simultaneously shows `PLAYING produced=95 rendered=95 dropped=0`, confirming the replay pipeline was ALSO painting correctly once `onDraw` could fire, with production and consumption keeping pace under this ordinary on-screen redraw cadence.

**No re-run of N1-C conformance goldens required:** this defect and its repair are entirely inside `LiveVtoTestRenderView`'s View lifecycle (whether `onDraw` is invoked at all). `LiveVtoGeometryPipeline`, `LiveVtoGarmentAttachment`, and `LiveVtoDeformation` -- the entire geometry and conformance surface -- are untouched. The JVM conformance suite continues to measure the identical, unchanged geometry computation; only whether that already-correct geometry reaches the screen has changed.
