...
| `WATCHLIST_WORKER_SECRET` | `commerce-watch-refresh` | required to run the Tier 2 sweep | internal | The refresh sweep is unreachable: `requireWorkerSecret()` refuses without it, so every Watch stays un-refreshed. Verified by name/digest via `supabase secrets list` (2026-09-05): **present on staging** (set 2026-09-01), **absent on production**. Enabling the sweep additionally requires `app_config.watchlist_worker_enabled`, seeded `false`. Production scheduling cannot be activated until this is set on the production project. |
| `EXPO_PUSH_ACCESS_TOKEN` | `commerce-watch-refresh` (`pushDelivery.ts`) | optional | Expo | Absent on both projects (verified by name 2026-09-05). The sender adds `Authorization: Bearer` only when set, so Expo Push still accepts sends without it. Required **only** if the Expo project later enables enhanced push security, at which point every send would 401 — set it before flipping that Expo setting, not after. |
| `GOOGLE_SERVICES_JSON` | Android build (EAS **file** secret, not a Supabase secret) | required for the `production` and `staging-certification` Android profiles | Firebase | Materialised by EAS into `android/app/google-services.json` at build time and consumed by the Google Services Gradle plugin; never committed. Verified present by name for the EAS `production` and `preview` environments (2026-09-05); absent for `development`, which is expected. Absent for a push-enabled profile, `android/app/build.gradle` now **fails the build closed** (NOTIF-02) rather than shipping an artifact that requests POST_NOTIFICATIONS and can never deliver. |
