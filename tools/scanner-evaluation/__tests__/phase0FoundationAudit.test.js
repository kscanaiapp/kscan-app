...
test('frozen verifier reproduces all inputs and all 56 governed image hashes', () => {
  ...
  // Superseded c3b68956… when the governed text digests were corrected to
  // LF-normalised bytes. Ground truth is untouched; see phase1FreezeHashPortability.
...
test('execute mode records an incomplete zero-call run when the hard attempt ceiling is reached', async () => {
  const outputDir = path.join(STORAGE_ROOT, 'results', 'phase0-execute-refusal');
  ...
  const result = withStorageRoot(() => runBaseline.main([
      ...
      '--certified-snapshot', path.join(STORAGE_ROOT, 'snapshots', 'certified-v140-f5f4ed2'),
    ], {
      executor: () => { adapterInvocations += 1; },
      countTokens: ({ model, call }) => ({
        inputTokens: 1000,
        serializedRequestPayload: JSON.stringify({ model, imageHash: call.imageHash }),
  ...
  process.exitCode = 0;
  assert.equal(result.ok, false);
  assert.equal(result.completedCaseCount, 0);
  assert.equal(result.refusedCaseIds.length, 1);
  assert.equal(result.reservation.totalGenerateAttempts, 0);
  assert.equal(adapterInvocations, 0);
