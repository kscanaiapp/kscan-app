...
const { ROOT, sha256OfTextFile } = require('./lib/governedStorage');
...
// The review briefs and reviewer locks recorded the guide digest over the bytes
// the reviewers actually read, which on a Windows working copy are CRLF. That
// value is the chain-of-custody record and is checked as-is. The value published
// in the freeze and seal is LF-normalised so it reproduces from a fresh clone.
const guideHashAsReviewed = sha256(guide.raw);
const guideHash = sha256OfTextFile(guide.absolute).slice('sha256:'.length);
if (guideHashAsReviewed !== review.value.guideSha256
    || guideHashAsReviewed !== holdoutBrief.value.guideSha256) {
...
const files = Object.fromEntries(governedFiles.map((absolute) => [
  repoRelative(absolute),
  sha256OfTextFile(absolute).slice('sha256:'.length),
]));
...
const seal = {
  ...
  reviewArtifactSha256: sha256OfTextFile(review.absolute).slice('sha256:'.length),
  ...
  labelingGuideSha256AsReviewed: guideHashAsReviewed,
  labelingGuideHashBasis: 'labelingGuideSha256 is over LF-normalised bytes and reproduces from a fresh clone. labelingGuideSha256AsReviewed is the working-copy digest recorded in the review briefs and reviewer locks. The guide CONTENT is identical; only the line-ending encoding differs.',
