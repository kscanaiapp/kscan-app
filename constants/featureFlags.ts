 ...
 *
 * Converged from the accepted Voice Scan lineage (PR #218,
 * feature/android-build34-voice-commerce-v1). It replaces the hardcoded
 * `false` this file carried while Voice Scan was unimplemented on this
 * lineage; that constant was NOT a policy decision to keep Voice off
 * forever, it was the absence of an implementation to switch on.
 *
 ...
 * switch, not the product entitlement boundary. Turning this on WITHOUT an
 * active K+ entitlement grants nothing: useVoiceScan refuses to start a
 * session (NOT_KPLUS) and VoiceScanButton routes the tap to the upgrade
 * sheet instead.
 *
 * NATIVE NOTE (Android): this flag alone cannot make Voice work on Android.
 * android/app/src/main/AndroidManifest.xml removes RECORD_AUDIO; the
 * certification artifact re-grants it through the governed
 * `kscan.voiceCertification` Gradle selector (android/app/build.gradle and
 * android/app/src/certification/AndroidManifest.xml). Flag ON without that
 * selector yields an "on-device recognition unavailable" path, never a
