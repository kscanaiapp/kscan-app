...
test('adapter: normalizeScanIdentifyResponse preserves detectedGarments', () => {
  const adapter = loadAdapter({});
  const out = adapter.normalizeScanIdentifyResponse({
    status: 'completed',
    recommendedProducts: [],
  ...
  assert.equal(out.detectedGarments.length, 2);
  assert.equal(out.detectedGarments[0].candidateId, 'garment-1-blazer');
  assert.equal(out.detectedGarments[1].label, 'Black trousers');
});
