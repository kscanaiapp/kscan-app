...
test('every surface that joins taxonomy de-duplicates it', () => {
  ...
  const joiners = [
    ...
    'components/scan-results/MultiItemResultNavigator.tsx',
  ];
...
test('the multi-item navigator de-duplicates its card AND its accessibility label', () => {
  // The label is a second rendering of the same fields; a screen reader hearing
  // "top, top, black" is the same defect.
  const source = fs.readFileSync(
    path.join(ROOT, 'components/scan-results/MultiItemResultNavigator.tsx'),
    'utf8',
  );
  const uses = source.match(/describeCandidate\(/g) ?? [];
  assert.ok(uses.length >= 3, 'both call sites plus the definition must use one helper');
  assert.equal(
    /\[candidate\.category, candidate\.subtype, candidate\.primaryColor\]\s*\n?\s*\.filter\(Boolean\)/.test(
      source,
    ),
    false,
    'the raw un-deduplicated join must be gone',
  );
});
