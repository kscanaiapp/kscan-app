 ...
 *
 * WHY THIS EXISTS. stagingImpact answers "is this release content worth
 * running staging validation for at all" and is deliberately blunt on
 * purpose: a mobile-only diff is correctly stagingImpact=true (it tags WEB
 * via app/ and components/, and WEB is a STAGING_IMPACT_TAG). That same
 ...
 * no-op once inside.
 *
 * PORT NOTE. Upstream (PR #238, on staging/production-parity) these fields sit
 * alongside a releaseClass/CONTROL_PLANE_PATTERNS classifier (DEF-REL-005)
 * that has not landed on this integration line. The four new fields are
 * computed identically here and are independent of releaseClass; the
 * stagingImpact cross-checks below are pinned to THIS line's real behavior
 * rather than staging/production-parity's, and there is no releaseClass to
 * assert against.
 *
...
test('mobile-only diff: no backend/edge deployment authority, but mobile runtime impact is true', () => {
  ...
  // Unchanged existing contract: a mobile-only diff still has stagingImpact
  // true here (app/ and components/ also carry the WEB tag), which is exactly
  // the bluntness backendDeploymentRequired exists to sit orthogonally beside.
...
test('docs-only diff: every deployment-applicability field is false', () => {
  ...
  assert.equal(out.migrationValidationRequired, false);
  assert.equal(out.mobileRuntimeImpact, false);
  // Unchanged existing contract: a docs-only diff has no staging impact.
...
test('backendDeploymentRequired is a strict subset of stagingImpact, so swapping the deploy gate can only narrow it', () => {
  // security-staging-gate.yml's deploy-staging/staging-health/synthetic-tests
  // jobs were moved from staging_impact to backend_deployment_required. That
  // is only safe if the latter implies the former -- otherwise the swap would
  // authorize a staging write that staging_impact would have refused. Both
  // source tags ('SUPABASE FUNCTION', 'DATABASE MIGRATION') are members of
  // STAGING_IMPACT_TAGS, which this pins behaviorally.
  for (const files of [
    ['app/index.tsx'],
    ['supabase/functions/foo/index.ts'],
    ['supabase/migrations/20260101000000_foo.sql'],
    ['supabase/functions/foo/index.ts', 'app/index.tsx'],
    ['docs/readme-note.md'],
    ['server.js'],
    ['.github/workflows/security-code.yml'],
  ]) {
    const out = classify(files);
    if (out.backendDeploymentRequired) {
      assert.equal(out.stagingImpact, true, `backendDeploymentRequired without stagingImpact for ${files.join(',')}`);
    }
  }
});
