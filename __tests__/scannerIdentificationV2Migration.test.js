...
test('governance: Scanner always sends identify_and_shop, never identify_for_style', () => {
  ...
  const built = adapter.buildScannerV2Request({
    mode: 'detect_items', evidence: evidenceFor(), platform: 'ios', requestId: 'r1',
  });
...
test('flag off: the legacy request is sent unchanged and carries no V2 envelope', async () => {
  ...
  const outcome = await runner.runScannerIdentification({
    mode: 'detect_items', evidence: evidenceFor(), platform: 'ios',
...
test('flag on: a V2 envelope is sent with the correct contract, intent and mode', async () => {
  ...
  assert.equal(body.source.platform, 'ios');
...
test('fallback: the retry reuses the same derivative with no recompression', async () => {
  ...
  await runner.runScannerIdentification({
    mode: 'detect_items', evidence, platform: 'ios', requestId: 'r1', sessionFlag: enabledFlag,
  });
...
test('fallback: requests are sequential, never parallel', async () => {
  ...
  await runner.runScannerIdentification({
    mode: 'detect_items', evidence: evidenceFor(), platform: 'ios', requestId: 'r1', sessionFlag: enabledFlag,
  });
...
test('evidence: each request carries exactly one evidence payload', async () => {
  const requests = [];
  const runner = loadRunner(async (image, options) => {
...
test('single-image: the session correlation binds selection to its own evidence', () => {
  const hook = read('hooks/useKScan.js');
  // The session owns exactly one evidence id, minted with the session and
  // reused verbatim through selection.
  assert.match(hook, /evidenceId: createEvidenceId\(\)/);
  assert.match(hook, /evidenceId: session\.evidenceId/);
  // Selection looks the candidate up by BOTH candidate id and evidence id.
  assert.match(
    hook,
    /entry\.candidateId === candidate\.id[\s\S]{0,80}entry\.evidenceId === evidence\.evidenceId/,
  );
  // The prepared derivative is reused, never re-prepared for selection.
  assert.match(hook, /preparedImage: session\.preparedImageUri/);
  assert.doesNotMatch(
    hook.slice(hook.indexOf('analyzeSelectedCandidate')),
    /compressForUpload|sanitizeImageBeforeUpload/,
    'selection must never recompress or re-sanitize',
  );
});

test('correlation: selection uses the candidate identity, never array position', async () => {
  const sent = [];
  const runner = loadRunner(async (image, options) => {
...
test('response: purchase options remain available from the legacy-compatible field', async () => {
  ...
  const outcome = await runner.runScannerIdentification({
    mode: 'identify_selected_item',
    evidence: evidenceFor('evidence-tuple-01'),
    platform: 'ios', requestId: 'r1', sessionFlag: enabledFlag,
    selectedCandidate: { evidenceId: 'evidence-tuple-01', candidateId: 'c1', category: 'Top' },
  });
...
test('isolation: Elise and StyleChat never construct a Scanner V2 request', () => {
  const eliseFiles = [
    ...
    'services/style-chat/eliseVisualContextEvidence.ts',
    'services/style-chat/eliseDirectImageAttachment.ts',
...
test('telemetry: only bounded, non-identifying fields are recorded', () => {
  const telemetry = adapter.buildScannerV2Telemetry({
    ...
    mode: 'identify_selected_item', evidenceSource: 'gallery', platform: 'ios',
...
test('telemetry: the only fallback reason is unsupported_version', () => {
  const withFallback = adapter.buildScannerV2Telemetry({
    ...
    evidenceSource: 'camera', platform: 'ios', result: null, fallbackUsed: true,
  });
  ...
  const noFallback = adapter.buildScannerV2Telemetry({
    enabled: true, attempted: true, accepted: false, mode: 'detect_items',
    evidenceSource: 'camera', platform: 'ios', result: null, fallbackUsed: false,
  });
