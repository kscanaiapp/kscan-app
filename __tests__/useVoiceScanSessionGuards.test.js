...
function loadUseVoiceScanWithMocks(overrides = {}, sourceOverride = null) {
  const {
    ...
    fetchVoiceCapabilities = async () => ({ supported: true, onDeviceAvailable: true, platform: 'ios' }),
...
// ── Fix 217-B: the startup sequence must not let a rejection escape ────────

test("217-B: fetchVoiceCapabilities rejecting resolves startSession (never throws) and reports RECOGNIZER_ERROR", async () => {
...
test('217-B: requestVoiceRecordingPermission rejecting resolves startSession (never throws) and reports RECOGNIZER_ERROR', async () => {
  const hook = loadUseVoiceScanWithMocks({
...
test('217-B: beginVoiceListening rejecting resolves startSession (never throws) and reports RECOGNIZER_ERROR', async () => {
  const hook = loadUseVoiceScanWithMocks({
...
test('217-B: capability-unavailable clears the active session id before returning the terminal state', async () => {
  const hook = loadUseVoiceScanWithMocks({
    fetchVoiceCapabilities: async () => ({ supported: true, onDeviceAvailable: false, platform: 'ios' }),
  });
  await hook.startSession();
...
test('217-B: permission-denied clears the active session id before returning the terminal state', async () => {
  const hook = loadUseVoiceScanWithMocks({
...
test('217-B: a stale attempt (cancelled mid-flight) whose fetchVoiceCapabilities rejects LATE is ignored, not reported', async () => {
...
test('217-B: a stale attempt (K+ lost mid-flight) whose beginVoiceListening rejects LATE is ignored, not reported', async () => {
  let rejectBegin;
  const hook = loadUseVoiceScanWithMocks({
    isKPlusActive: true,
...
test('217-B: a replacement session started after the first goes stale is never clobbered by the first attempt failing late', async () => {
  let rejectFirst;
  const hook = loadUseVoiceScanWithMocks({
...
// ── Fix 217-C: a rejected native-end transition must never apply a result ──

test('217-C: a genuine in-flight session (real happy path) still reaches reviewing with the transcript', async () => {
...
test('217-C: wrong session id is ignored (a stray event from an old/replaced session)', async () => {
  const hook = await startedListeningSession();
...
test('217-C REQUIRED NEGATIVE CONTROL: a late onSessionEndedByNative callback after cancel cannot repopulate a Voice draft', async () => {
  const hook = await startedListeningSession();
  const sessionId = hook.activeSessionId;
...
test('217-C NEGATIVE CONTROL: removing the state-is-listening guard lets a stale-state callback populate the draft', async () => {
