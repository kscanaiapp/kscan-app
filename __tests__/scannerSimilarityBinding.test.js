...
test('ISOLATION — an explicitly undefined actorId collapses to null, never unfiltered', async () => {
  const { mod, calls } = setup();
  const binding = mod.buildScannerSimilarityBinding({
    actorId: undefined, scanId: 'scan-1', garment: GARMENT,
  });
  await binding.loadClosetRecords();
  await binding.loadRecentScanRecords();

  // null = the ownerless device-local partition. undefined = every account.
  assert.equal(calls.closet[0].actorId, null);
  assert.equal(calls.library[0].actorId, null);
});
