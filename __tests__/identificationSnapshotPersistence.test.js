...
/** iOS keeps its purchase-option helpers in dressingRoomCommerce. */
function loadCommerceModule() {
  return transpile('services/dressingRoomCommerce.ts', {
    '../types/canonicalDressingRoomItem': {},
  });
}

function loadLibraryModule() {
  const commerce = loadCommerceModule();
...
function loadCloudModule() {
  return transpile('services/savedScansCloud.ts', {
    ...
    './dressingRoomCommerce': loadCommerceModule(),
...
test('IMG-008: a fresh client hydrates the full snapshot from the cloud row', () => {
  ...
  // PLATFORM DIVERGENCE: iOS supports a durable ownerless (signed-out) local
  // partition and its row mapper deliberately does not project ownerId onto the
  // model; the row's user_id remains authoritative. Assert the property that
  // matters on both platforms — ownership is never taken from the payload.
  assert.ok(
    model.ownerId === undefined || model.ownerId === 'user-1',
