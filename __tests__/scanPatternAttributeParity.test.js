...
test('a reopened Recent Scan passes its persisted pattern to the detail view', () => {
  ...
  //
  // Build 28 read `selectedScan.attributes.pattern` straight into the detail
  // view. Build 29 resolves every fashion field through the canonical resolver
  // and hands the detail view `selectedOwnedItem`, which is strictly better --
  // one authority instead of a per-field read. Pinning the old expression would
  // have forced the newer architecture back to the older one, so what is
  // asserted here is the BEHAVIOUR both share: a reopened scan shows the
  // pattern it was saved with.
  ...
  assert.match(
    ...
    'the detail view must receive pattern from the canonical owned-item projection',
  );

  // The behavioural half, run against the REAL resolver rather than asserted
  // from source: a legacy scan with NO identification snapshot must still
  // resolve its pattern from the saved attributes. This is where Build 29
  // actually lost it -- the canonical legacy projection omitted `pattern`
  // entirely, so there was nothing for the resolver to fall back to.
  const canonical = runModule('services/canonicalFashionMetadata.ts');
  const resolved = canonical.resolveCanonicalFashionMetadata({
    snapshotV2: null,
    snapshot: null,
    legacy: {
      category: 'Outerwear',
  ...
  assert.equal(resolved.pattern, 'Herringbone', 'a legacy scan must resolve its saved pattern');
  assert.equal(resolved.provenance.pattern, 'legacy_metadata');

  // And the projection that feeds it must actually pass the field along.
  const owned = read('services/ownedClosetItems.ts');
  assert.match(
    owned,
    /pattern: attributes\.pattern,/,
    'the canonical legacy projection must include pattern, or a reopened legacy scan loses it',
  );
});
