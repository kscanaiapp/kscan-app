/**
 * IOS29-NEW-003 / EDGE-02 — server posture for the two Apple Edge Functions and
 ...
 *
 * Recovered under EDGE-02 (govern Apple credential functions under source
 * authority) from the certified revocation lineage (git commit e369fca9,
 * originally on hotfix/ios-build29-apple-revocation). One assertion below was
 * adapted from the original: the original diffed migrations against
 * `origin/hotfix/ios-build29-appstore-readiness`, a branch specific to that
 * feature line's own topology and meaningless from this branch's ancestry
 * (which shares no merge-base worth diffing against). The forward-only
 * property it protected is asserted directly on migration content instead.
 */
...
/**
 * Strips comments so a guard call cannot satisfy a source-text assertion by
 * being commented out. EDGE-NC-002 proved this matters: `// requireServiceRole(req);`
 * still matches a plain `/requireServiceRole\(req\)/` regex, so the raw
 * un-stripped test passed against source with the guard call disabled.
 */
...
test('revocation refuses every caller that is not holding the service-role key', () => {
  ...
  // Comment-stripped so a disabled guard (e.g. `// requireServiceRole(req);`)
  // cannot satisfy this by leaving the matching text present but inert.
  const live = readSourceWithoutComments(REVOKE_FN);

  assert.match(live, /requireServiceRole\(req\)/, 'the guard call must be live, not commented out');
  assert.match(
    source,
  ...
  const guardIndex = live.indexOf('requireServiceRole(req)');
  const bodyIndex = live.indexOf('await req.json()');
...
test('the migration is forward-only: additive DDL, no edit of prior schema history', () => {
  // Adapted for this branch (see file header): asserts the property directly
  // on migration content instead of diffing against the original feature
  // line's own branch, which is unrelated to this branch's ancestry.
  const migration = readSource(MIGRATION);

  assert.match(
    migration,
    /create table if not exists public\.apple_auth_credentials/,
    'the table must be created idempotently, never dropped and recreated',
  );
  assert.ok(
    !/\bdrop\s+table\b|\balter\s+table\s+public\.\w+\s+rename\b/i.test(migration),
    'a forward-only migration must not drop or rename existing objects',
  );
});
