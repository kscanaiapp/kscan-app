...
test('the migration is forward-only and does not edit applied history', () => {
  const migration = readSource(MIGRATION).toLowerCase();
  assert.doesNotMatch(migration, /\bdrop\s+table\b|\btruncate\b|\bdelete\s+from\b/);
  assert.match(migration, /create table if not exists public\.apple_auth_credentials/);
});
