 ...
 * 20260916233708_reaction_counts_bind_anonymous_share_token.sql, which is the
...
const MIGRATION = 'supabase/migrations/20260916233708_reaction_counts_bind_anonymous_share_token.sql';
...
test('B34-SEC-003: the corrected migration is the last word on the RPC', () => {
  ...
  assert.equal(
    latest.file,
    '20260916233708_reaction_counts_bind_anonymous_share_token.sql',
...
test('B34-SEC-003: the deployed contract admits exactly one signature, so no client can fall back', () => {
  // WHY THIS ASSERTS THE MIGRATION AND NOT THE CLIENT. This file arrived with
  // PR #435, which bundled the client-side token threading together with the
  // migration. On THIS branch -- the backend authority line -- that client
  // change deliberately does not exist: the app ships from
  // release/kscan-pre-freeze-v1 and release/ios-build34-testflight-staging-v2,
  // and importing a client repair here to satisfy an assertion would collapse
  // two authority lines that are kept apart on purpose.
  //
  // The property that is genuinely THIS branch's to guarantee is that the
  // backend leaves a stale client no way to keep working: the one-argument
  // overload is dropped rather than kept alongside the new one, so a caller
  // that sends item ids alone gets an error instead of silently resolving to
  // the old permissive anonymous predicate. That is asserted below and in
  // 'the old single-argument signature is dropped, not overloaded'.
  //
  // The client half -- every public-screen call carrying the validated route
  // token and no owner-screen call carrying one -- is asserted against the
  // real client source by __tests__/dressingRoomReactionShareTokenBinding.test.js
  // on both release lines, where that code actually lives.
  const migration = read(MIGRATION);
  assert.match(
    migration,
    /drop function if exists public\.get_item_reaction_counts\(uuid\[\]\)/i,
    'the single-argument overload must be dropped so no stale client can reach the old predicate',
  );
  assert.doesNotMatch(
    migration,
    /create (or replace )?function public\.get_item_reaction_counts\(\s*p_item_ids uuid\[\]\s*\)/i,
    'no one-argument definition may be recreated',
  );
});
