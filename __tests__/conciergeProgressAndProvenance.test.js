...
test('UX-1: the indicator is chosen from the SENT message, not the composer', () => {
  ...
  //
  // CONVERGENCE #282 + #284. #282 rewrote this effect to also recover a durable
  // draft, which added an EARLY RETURN and a third dependency (user?.id). The
  // anchors below are re-cut onto that shape, and the assertion is stronger
  // than the one it replaces: it is no longer enough for the clear to appear
  // somewhere in the effect, it must appear BEFORE the early return. A clear
  // placed after it would be skipped for exactly the conversations that already
  // hold a composer draft -- which is most of them.
  const resetStart = screen.indexOf('const inMemory = getDraftComposerText(stableSessionId, actorKey);');
  const resetEnd = screen.indexOf('}, [stableSessionId, actorKey, user?.id]);');
  assert.ok(
    resetStart !== -1 && resetEnd > resetStart,
    'the per-conversation reset effect was not found: re-anchor this assertion rather than '
      + 'letting an empty slice decide the invariant',
  );
  const reset = screen.slice(resetStart, resetEnd);
  assert.ok(reset.includes("setSentMessageText('')"), 'the in-flight message must be cleared here');

  const clearAt = reset.indexOf("setSentMessageText('')");
  const earlyReturnAt = reset.indexOf('return;');
  assert.ok(earlyReturnAt !== -1, 'the draft-recovery early return was expected in this effect');
  assert.ok(
    clearAt < earlyReturnAt,
    'the in-flight message must be cleared unconditionally, before the draft-recovery '
      + 'early return, or a conversation with a draft in memory keeps the previous '
      + "conversation's wait copy",
  );
});
