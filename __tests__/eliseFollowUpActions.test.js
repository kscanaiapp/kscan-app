...
test('follow-ups submit through the one existing send pipeline', () => {
  ...
  assert.match(screen, /onSend=\{\(text\) => submitMessage\(text, \{ clearComposer: true \}\)\}/);
  assert.match(screen, /submitMessage\(prompt, \{ clearComposer: false \}\)/);
  assert.equal(
    (screen.match(/void sendMessage\(/g) ?? []).length,
    2,
    'the attachment branch and the text branch are the only two sends',
  );
