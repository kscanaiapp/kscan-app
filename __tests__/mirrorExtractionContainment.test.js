...
test('no model asset was added to the repository', () => {
  // TRACKED FILES ONLY. Walking the working tree also walks `modules/.../build`,
  // which is Gradle output full of vendor protobufs — untracked, gitignored, and
  // regenerated by any local compile. "Added to the repository" is a statement
  // about what git carries, so git is what gets asked.
  const { execFileSync } = require('node:child_process');
  const tracked = execFileSync('git', ['ls-files', 'assets', 'modules'], {
    cwd: ROOT,
    encoding: 'utf8',
    ...
    .split('\n')
    .filter(Boolean);
  const found = tracked.filter((rel) => /\.(tflite|mlmodel|mlmodelc|onnx|pb|pt|task)$/.test(rel));
