...
test('the native extension is additive: face masking is untouched', { skip: !NATIVE_MODULE_PRESENT && 'no local native module on this platform line' }, () => {
  ...
  assert.ok(kotlin.includes('AsyncFunction("detectAndMaskFaces")'), 'face masking was disturbed');
  assert.ok(kotlin.includes('AsyncFunction("detectPersonRegions")'));
  assert.ok(kotlin.includes('AsyncFunction("getExtractionCapabilities")'));
});

test('the authorized runtime, and only the authorized runtime, is declared', { skip: !NATIVE_MODULE_PRESENT && 'no local native module on this platform line' }, () => {
...
test('no model asset was added to the repository', () => {
  const found = [];
  function walk(dir) {
    ...
    for (const entry of fs.readdirSync(path.join(ROOT, dir), { withFileTypes: true })) {
      ...
      const rel = `${dir}/${entry.name}`;
      if (entry.isDirectory()) walk(rel);
      else if (/\.(tflite|mlmodel|mlmodelc|onnx|pb|pt|task)$/.test(entry.name)) found.push(rel);
    }
  }
  walk('assets');
  walk('modules');
  assert.deepEqual(found, [], `model assets present: ${found.join(', ')}`);
});
...
test('the extraction runtime is reachable only where the native module actually exists', () => {
  ...
  if (!NATIVE_MODULE_PRESENT) {
    // Recorded, not silently tolerated: this platform line cannot extract until
    // the module is propagated, which is an owner decision.
    assert.ok(
      !exists('modules'),
      'a partial local module directory appeared; the module is all-or-nothing',
    );
  }
});
