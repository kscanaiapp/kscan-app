...
test('iOS direct image resolution no longer manufactures a saved_scan dependency', () => {
  const direct = read('services/style-chat/eliseDirectImageAttachment.ts');

  assert.match(direct, /createClosetCandidate/);
  assert.match(direct, /promoteSelectedClosetCandidates/);
  assert.doesNotMatch(direct, /saveScanToCloud|upsertSavedScanRowForAttachment/);
  assert.doesNotMatch(direct, /ensureSavedScanMediaBacking/);
});
...
test('abandoned sends recover and sent focus cannot block the next attachment', () => {
  ...
  assert.match(
    attachments,
    /state === 'sent'[\s\S]*getFocusedDraftId\(sessionId\)[\s\S]*setFocusedDraftId/,
  );
});

test('legacy intake retries have one native picker launch owner', () => {
  const intake = read('components/style-chat/StyleChatPhotoIntake.tsx');

  assert.doesNotMatch(intake, /resetState\(\);\s*void startPicker\(\)/);
  assert.match(intake, /onPress=\{resetState\}/);
  assert.match(
    intake,
