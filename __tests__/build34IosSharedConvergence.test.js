/**
 * Build 34 iOS — SHARED CLIENT CONVERGENCE closure (storage layer).
 *
 * This is the iOS counterpart of
 * __tests__/build34SharedClientConvergence.test.js on the Android/shared line.
 * It asserts the SAME converged services/free-tier/freeTierStorage.ts against
 * the real module graph, because that module is now byte-identical on both
 * lines and a divergence between them is exactly what this campaign existed to
 * remove.
 *
 * WHAT IS DELIBERATELY ABSENT, AND WHY IT IS NOT AN OVERSIGHT. The Android
 * suite also drives the purge through services/deletion/ownerTerminalPurge.ts,
 * the terminal-deletion orchestrator. That module -- and the whole
 * services/deletion/ subsystem it belongs to: the status client, the status
 * receipt, the pending-deletion marker store, the terminal decision and the
 * reconciler -- DOES NOT EXIST on the frozen Build 34 iOS release candidate.
 * That is not an accident of this branch either: services/accountDeletion.js
 * says so in its own contract comment --
 *
 *   "Nothing here may purge local Recent Scans or unlink media -- that stays
 *    gated behind the terminal-status endpoint, which is not built yet."
 *
 * -- and the three owner-scoped purge primitives that DO exist here
 * (purgeLocalScansForOwner, purgeLocalClosetForOwner,
 * purgeLocalClosetCandidatesForOwner) have zero production callers for the
 * same reason. There is no terminal completion path on this line to wire
 * anything into.
 *
 * So this file asserts the half that IS this line's to guarantee: that the
 * owner-specific primitive `clearFreeTierStoresForActor` exists, is correct,
 * and satisfies every safety property the terminal path will require of it --
 * explicit actor, no ambient default, no reach into another partition, no
 * device-wide clear, idempotent. It is ready for the terminal path whenever
 * that path is built. What it is NOT is silently wired to some other trigger:
 * firing it at deletion SUBMISSION would destroy data during the 30-day
 * restorable grace window, which is a worse defect than the one it closes.
 *
...
test('CPR-FT-001 PRIMITIVE: the owner-scoped purge physically removes that actor\'s free-tier namespace', async () => {
  const app = boot();
  ...
  // On this line the call is to the PRIMITIVE, not to a terminal orchestrator:
  // services/deletion/ does not exist here (see the file header). What is
  // asserted is that the primitive erases, so the terminal path has something
  // correct to call once it is built.
  app.signIn(USER_B);
  const result = await app.freeTier.clearFreeTierStoresForActor(USER_A);
  assert.equal(result.ok, true, 'the owner-scoped purge must report success');
...
test('CPR-FT-001 PRIMITIVE: the purge takes the named actor\'s stores and NOTHING else on the device', async () => {
  const app = boot();
...
test('PURGE -> RELAUNCH -> B: a fresh process after an owner purge hands B nothing of A\'s', async () => {
  const shared = new Map();

  const first = boot({ storage: shared });
  await seedAllStores(first, USER_A);
  first.signOut();
  await first.freeTier.clearFreeTierStoresForActor(USER_A);
...
test('UPGRADE: adopted legacy data is owned, survives a relaunch, and is erased by deletion', async () => {
  ...
  relaunched.signIn(USER_B);
  await relaunched.freeTier.clearFreeTierStoresForActor(USER_A);
