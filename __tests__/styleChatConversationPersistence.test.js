...
test('HISTORICAL_ASSISTANT_SPEECH: reopening a greeted conversation claims no speech attempt', async () => {
  const harness = createHarness();
  const session = await harness.repository.createStyleChatSession({});

  // First entry: greeting is inserted and is eligible to speak exactly once.
  const inserted = await harness.greeting.ensureSessionGreeting(USER_A, session.id, 'Hello, I am Elise.');
  assert.equal(inserted.inserted, true);
  harness.greeting.noteInsertedGreetingForSpeech(USER_A, session.id, inserted.message.id);
  assert.equal(harness.greeting.claimGreetingSpeechAttempt(USER_A, session.id), inserted.message.id);
  assert.equal(
    harness.greeting.claimGreetingSpeechAttempt(USER_A, session.id),
    null,
    'The welcome must not speak twice in one process',
  );
  ...
  assert.equal(
    restartedGreeting.claimGreetingSpeechAttempt(USER_A, session.id),
    null,
    'Rehydrated history must not become eligible for speech',
  );
});
...
test('HISTORICAL_ASSISTANT_SPEECH: a conversation with replies is never retro-greeted or spoken', async () => {
  ...
  assert.equal(harness.greeting.claimGreetingSpeechAttempt(USER_A, session.id), null);
...
test('LOGOUT: greeting/speech state is clearable at the auth boundary', async () => {
  const harness = createHarness();
  const session = await harness.repository.createStyleChatSession({});
  const inserted = await harness.greeting.ensureSessionGreeting(USER_A, session.id, 'Hello.');
  harness.greeting.noteInsertedGreetingForSpeech(USER_A, session.id, inserted.message.id);
  ...
  assert.equal(harness.greeting.getPendingGreetingSpeechMessageId(USER_A, session.id), null);
});
...
test('PENDING_GENERATION: leaving mid-generation keeps one user message and one reply', async () => {
  const harness = createHarness();
  const session = await harness.repository.createStyleChatSession({});
  const userMessage = await harness.repository.saveStyleChatMessage({
    sessionId: session.id,
    sender: 'user',
    content: 'Style this for me',
  });

  // The reply lands after the user has navigated away.
  await harness.repository.saveStyleChatMessage({
    sessionId: session.id,
    sender: 'assistant',
  ...
  // Returning re-drives the same turn; the source-message constraint collapses it.
  const replayed = await harness.repository.saveStyleChatMessage({
    sessionId: session.id,
    sender: 'assistant',
    content: 'Here is the look.',
    sourceMessageId: userMessage.id,
  ...
  assert.equal(history.length, 2, 'A resumed generation must not append a second reply');
  assert.equal(history.filter((m) => m.sender === 'user').length, 1);
  assert.equal(replayed.id, history[1].id);
});
...
test('HISTORICAL_ASSISTANT_SPEECH: resuming past newer empty stubs still claims no speech attempt', async () => {
  const harness = createHarness();
  const real = await harness.repository.createStyleChatSession({});
  await harness.greeting.ensureSessionGreeting(USER_A, real.id, 'Hello, I am Elise.');
  ...
  assert.equal(
    restartedGreeting.claimGreetingSpeechAttempt(USER_A, resumedId),
    null,
    'the resumed conversation must not become eligible for greeting speech',
  );
});
