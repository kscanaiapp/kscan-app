...
test('no store function accepts an actor id', () => {
  ...
  //
  // ONE AUDITED EXEMPTION (Repair 07): purgeDressingRoomInteractionsForActor.
  ...
  const source = fs.readFileSync(path.join(ROOT, 'services/privateDressingRoomInteractionStore.ts'), 'utf8');

  const takesActorId = (source.match(/export async function (\w+)\([^)]*actorId\s*:/g) || []).map(
    (match) => /function (\w+)/.exec(match)[1],
  );
  assert.deepEqual(takesActorId, ['purgeDressingRoomInteractionsForActor'], 'only the terminal-deletion purge may name an actor');

  // Destructive-only: it hands back how many records went, never their contents.
  const signature = new RegExp(
    'export async function ' + 'purgeDressingRoomInteractionsForActor' + '\\([^)]*\\): Promise<\\{ ok: boolean; removed: number \\}>',
  );
  ...
  for (const dir of ['app', 'components', 'hooks', 'contexts', 'services', 'stores']) {
    ...
    while (stack.length) {
      ...
      for (const entry of fs.readdirSync(current, { withFileTypes: true })) {
        ...
        else if (/\.(ts|tsx|js|jsx)$/.test(entry.name) && fs.readFileSync(full, 'utf8').includes('purgeDressingRoomInteractionsForActor')) {
  ...
  assert.deepEqual(
    callers.sort(),
    ['services/privateDressingRoomInteractionStore.ts', 'services/deletion/ownerTerminalPurge.ts'].sort(),
