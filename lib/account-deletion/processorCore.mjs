 ...
 * direction. Reads public.dressing_room_user_blocks directly with the service
 * role rather than calling internal.is_dressing_room_pair_blocked(): that
 * helper deliberately lives in the unexposed `internal` schema and holds no
 * EXECUTE grant for service_role, so the worker cannot invoke it.
 *
 * Fails CLOSED. A lookup error returns `blocked: true` so an unreadable block
 * relation can never be mistaken for an absent one -- the cost of a false
 * positive is a room that cascade-deletes with its owner, which is the
 * outcome the owner already chose; the cost of a false negative is handing a
 * blocked account somebody else's room.
 */
export async function isTransferPairBlocked(supabase, ownerUserId, candidateUserId) {
  const result = await supabase
    ...
    .limit(1);
  if (result.error) {
    // A table that does not exist yet (pre-blocking environments) is not a
    // failure to read a relation that does exist -- treat it as "no blocks".
    if (isMissingResourceError(result.error)) return { blocked: false, errored: false };
    return { blocked: true, errored: true };
  }
  return { blocked: Array.isArray(result.data) && result.data.length > 0, errored: false };
}

/**
 * Decides whether one participant may inherit a deleting owner's room.
 *
 * `participant` is the dressing_room_participants row, not just an id: the
 * transfer policy is "earliest remaining ACTIVE participant", and left_at is
 * what "active" means in this schema. A row with left_at set was marked
 * departed by block_dressing_room_user() (owner blocked participant,
 * participant blocked owner, or participant blocked a co-participant) and must
 * never be handed the room -- nor may an account the owner has an active block
 * against, which is checked independently so a block created without a left_at
 * consequence still disqualifies.
 */
export async function validateTransferCandidate(supabase, participant, ownerUserId) {
  const candidateUserId =
    typeof participant === 'string' ? participant : participant?.user_id ?? null;
  if (!candidateUserId) {
    return { valid: false, reason: 'candidate_missing' };
  }
  if (typeof participant === 'object' && participant !== null && participant.left_at) {
    return { valid: false, reason: 'participant_left_room' };
  }
  ...
  if (ownerUserId && ownerUserId !== candidateUserId) {
    const blockCheck = await isTransferPairBlocked(supabase, ownerUserId, candidateUserId);
    if (blockCheck.errored) {
      return { valid: false, reason: 'block_lookup_error' };
    }
    if (blockCheck.blocked) {
      return { valid: false, reason: 'pair_blocked' };
    }
  }
...
/**
 * Registers one storage prefix that finished a purge with objects still
 * RETAINED, so the scheduled orphan sweep can revisit it once the last
 * surviving dressing-room reference disappears.
 *
 * The RAW prefix is required here and must never leave this module: it embeds
 * the full deleted user id, which is exactly what addresses the objects, and is
 * the same identifier already present in every storage path the work item
 * points at. Every value this module RETURNS carries the shortUserId-sanitized
 * prefix instead.
 *
 * Best effort, matching the automated worker: a bookkeeping failure must never
 * fail a purge that already succeeded, and the objects stay retained -- the
 * safe direction -- until a later observation registers the prefix.
 */
async function recordRetainedOwnerMedia(supabase, deletionRequestId, bucket, rawPrefix, retained) {
  if (!deletionRequestId) return 'not_registered_no_request_id';
  try {
    const result = await supabase.rpc('record_retained_owner_media', {
      p_request_id: deletionRequestId,
      p_bucket: bucket,
      p_prefix: rawPrefix,
      p_retained: Number.isFinite(retained) ? retained : 0,
    });
    return result?.error ? 'registration_failed' : 'registered';
  } catch {
    return 'registration_failed';
  }
}

export async function deleteOwnedStorageObjects(supabase, userId, options = {}) {
  // INT-KPLUS-010 parity. deleteOwnedStorageObjects deliberately PRESERVES
  // objects a surviving dressing_room_items row still points at -- item rows
  // cascade with their ROOM, not with the deleting user, so a room transferred
  // to another owner keeps its images. Nothing then revisited those objects on
  // this path: the automated worker (supabase/functions/process-account-
  // deletions) has recorded retained prefixes since INT-KPLUS-010, but this
  // manual CLI path did not, so a CLI purge left retained media with nothing
  // pointing at it, no account to own it, and no work item telling the sweep
  // where to look. Passing deletionRequestId opts a caller into registration;
  // omitting it preserves the previous behaviour for callers that have no
  // ledger row (dry runs, tests).
  const deletionRequestId = options.deletionRequestId ?? null;
  const results = [];
    ...
    for (const prefix of resource.prefixesForUser(userId)) {
      ...
      if (removable.length === 0) {
        results.push({
          ...
          retainedRegistration: await recordRetainedOwnerMedia(
            supabase,
            deletionRequestId,
            resource.bucket,
            prefix,
      ...
      results.push({
        ...
        retainedReferenced: retained,
        // A prefix that retained nothing still calls through: the RPC deletes
        // any stale work item for it rather than leaving one behind.
        retainedRegistration: await recordRetainedOwnerMedia(
          supabase,
          deletionRequestId,
          resource.bucket,
...
export async function getSharedRoomsForUser(supabase, userId) {
  ...
  for (const room of rooms) {
    ...
    for (const participant of participants) {
      const validation = await validateTransferCandidate(supabase, participant, userId);
      const candidate = {
...
/**
 * Statuses that mean the Apple obligation is settled and the purge may proceed.
 * Mirrors the deployed apple-revoke-credential contract exactly
 * (supabase/functions/_shared/appleAuth/revocationGate.ts). `no_credential`
 * covers non-Apple accounts and legacy Apple accounts from before capture
 * existed; TN3194 is explicit that a missing token does not excuse deleting.
 */
...
export function isBlockingAppleRevocationStatus(status) {
  return !APPLE_REVOCATION_COMPLETE_STATUSES.includes(status);
}
 ...
 *
 * This deliberately calls the already-deployed function rather than
 * reimplementing Apple's REST contract: the client secret, ES256 signing key
 * and token-encryption key live only in that function's environment and are
 * unreachable from here. Nothing here logs a token, code, or Apple response.
 *
 * WHY THE AUTHORIZATION HEADER IS EXPLICIT (issue #390).
 * This function previously relied on the service-role key that the caller's
 * client was built with being forwarded as the Bearer token. That assumption
 * does not hold. supabase-js documents the Authorization header as "reserved
 * for the signed-in user's JWT", and from 2.111.0 onward a new-format API key
 * (`sb_publishable_…` / `sb_secret_…`) is deliberately NOT sent as a Bearer
 * token when the client has no session -- only the `apikey` header is.
 * apple-revoke-credential enforces its own `Authorization: Bearer
 * <SUPABASE_SERVICE_ROLE_KEY>` check, so the nested call began arriving
 * unauthenticated, was answered 403, and every purge -- including for accounts
 * with no Apple credential at all -- dead-ended in
 * `apple_revocation_blocked:failed`.
 *
 * A privileged service-to-service call therefore states its own credential
 * rather than inheriting one by SDK convention. The key is required, not
 ...
 */
export async function requestAppleRevocation(supabase, userId, serviceRoleKey) {
 ...
 *   6. Revokes the Sign in with Apple authorization, and STOPS if that is not
 *      settled. This step is built into the pipeline rather than exposed as an
 *      option so no caller can drop an Apple obligation by forgetting a hook.
 *   7. Calls options.beforeAuthDelete(supabase, request) if provided, so a
 *      caller can prepare a surviving record or write a ledger event before
 *      the Auth user (and its cascades) disappear.
 *   8. Deletes the Auth user.
 *   9. Treats "user not found" as a successful, idempotent delete.
 *  10. Calls options.afterAuthDelete(supabase, request) if provided.
 *  11. Returns a sanitized summary (no full user id, no email addresses).
 *
 * Step 6 sits immediately before the Auth delete because the credential row
 * cascades on that delete: after it the refresh token is gone and the
 * obligation can never be met. A retryable failure therefore has to stop the
 * purge here, with nothing yet erased that the retry would need.
 */
export async function runHardDeletePipeline(supabase, request, options = {}) {
  ...
  const storageResults = await deleteOwnedStorageObjects(supabase, userId, {
    deletionRequestId: request?.id ?? null,
  });
