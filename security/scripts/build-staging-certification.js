...
const NOT_APPLICABLE_ALLOWED = new Set(['migration_validation', 'staging_health', 'synthetic_auth']);
...
function build(input) {
  ...
  return {
    ...
    sha_match: shaMatch,
    ...components,
    blocking_findings: [...blocking.map(([name]) => name), ...(shaMatch ? [] : ['candidate_identity_mismatch']), ...invalid.map(([name]) => `invalid_verdict:${name}`), ...unexpectedNotApplicable.map(([name]) => `required_control_not_applicable:${name}`)],
    report_only_findings: reportOnly.map(([name]) => name),
    final_verdict: finalVerdict,
    promotion_eligible: finalVerdict === 'PASS' || finalVerdict === 'PASS_WITH_REPORT_ONLY_FINDINGS',
  };
}
