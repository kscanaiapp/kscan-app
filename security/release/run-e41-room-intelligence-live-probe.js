...
async function run(env, fetchImpl) {
  ...
  try {
    ...
    const report = {
      ...
      // Provenance, supplied by the TRUSTED workflow after resolving the
      // candidate through the GitHub API. The candidate's own source is never
      // checked out or executed, so this records WHAT was certified without
      // that code ever being part of the credential trust boundary. All values
      // are SHAs and ref names -- no secrets, no content.
      provenance: {
        candidateRefRequested: environment.E41_CANDIDATE_REF ?? null,
        candidateShaResolved: environment.E41_CANDIDATE_SHA ?? null,
        candidateTreeSha: environment.E41_CANDIDATE_TREE_SHA ?? null,
        probeTrustSourceSha: environment.E41_TRUST_SOURCE_SHA ?? null,
        probeWorkflowSha: environment.E41_WORKFLOW_SHA ?? null,
        candidateSourceExecuted: false,
      },
