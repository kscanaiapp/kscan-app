-- Backend contract fingerprint.
--
-- Produces one JSON document describing the application-facing backend contract
-- of whatever database it is run against, so a locally rebuilt baseline can be
-- compared to production object-by-object rather than by object counts.
--
-- Read-only. Safe to run against production.
--
-- Deliberately excludes: row data, auth/storage platform-managed objects that
-- Supabase provisions identically in every project, and anything under
-- supabase_migrations (history is not contract).
select jsonb_build_object(
  ...
  -- Grants are compared for the roles the released client actually authenticates
  -- as. postgres/supabase_admin differ by platform provisioning, not by contract.
  'table_grants', (
    select coalesce(jsonb_object_agg(k, v), '{}'::jsonb)
    from (
  ...
  'sequence_grants', (
    ...
    from (
      ...
      cross join lateral (
        select unnest(array['SELECT','USAGE','UPDATE']) as priv
      ) privs
