...
export function createCollabRequestId(): string {
  ...
  // Hermes has NO global Web Crypto: the previous fallback dereferenced
  // `crypto.getRandomValues` and threw "Property 'crypto' doesn't exist",
  // killing Room Chat sends AND DR-3 reaction mutations on iOS. Use the
  // already-installed expo-crypto native module instead (no polyfill).
  if (typeof ExpoCrypto.randomUUID === 'function') {
    return ExpoCrypto.randomUUID();
  }
  // RFC4122 v4 from expo-crypto secure bytes (same format, same collision
  // resistance, preserves the server's uuid idempotency contract).
 ...
 *
 * This exists so Block and Report User never depend on the target having sent
 * a message: a participant who only joined, reacted, or added items must still
 * be actionable, and Report & Hide must not be able to strand the only path.
 *
