...
export async function setReasonForMessage(params: {
  ...
  if (!userKey || !sessionId || !messageId) {
    throw new Error('Style DNA reason requires userKey, sessionId, and messageId.');
  }
  if (!isFeedbackValue(feedback)) {
    throw new Error('Invalid Style DNA feedback value.');
  }
  if (!isReasonValidForFeedback(reasonCode, feedback)) {
    throw new Error('Invalid Style DNA reason code for the given feedback.');
  }
