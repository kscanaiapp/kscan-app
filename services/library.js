  ...
  try {
    return await enqueueLibraryMutation(async () => {
      ...
      if (actorRequest !== undefined) {
        owner = normalizeOwnerId(normalizeActorIdArg(actorRequest.actorId));
      ...
      // never rolls back the committed local write. Android's saveScanToCloud
      // takes the expected actor id so it can detect an actor change under
      // the fire-and-forget call; pass it through when we have one.
      if (normalizeOwnerId(target.ownerId)) {
        saveScanToCloud(updated[index], undefined, owner || undefined).catch(() => null);
      }
      return true;
    });
  } catch {
