...
export async function getItemReactionCounts(
  ...
  // Anonymous public-share readers must present the specific live share token
  // for this room (B34-SEC-003) — the RPC's anonymous branch no longer admits
  // a caller on room state alone. Authenticated in-app callers never had a
  // token and do not need one; omitting it resolves through the authenticated
  // branch unchanged.
