...
test('end-to-end: a clean Easy source with a correct fidelity hint produces an eligible, valid asset with real texture/alpha pixels', async () => {
...
test('end-to-end: an unsupported category is rejected before any pixel work, with UNSUPPORTED_CATEGORY', async () => {
...
test('end-to-end: a skin-blob HARD source is rejected with OCCLUSION_TOO_HIGH and never produces a texture', async () => {
...
test('end-to-end: eligibility and rejection never diverge — a pure confidence-gate failure still populates manifest.rejection (PHASE4-007)', async () => {
...
test('end-to-end: a scattered-object scene is rejected with MULTIPLE_GARMENTS', async () => {
...
test('end-to-end: a wrong fidelity hint yields PRODUCT_FIDELITY_FAILED, never a silently-accepted wrong-color asset', async () => {
...
test('end-to-end: the same source processed twice yields byte-identical assetId (determinism, task section 28)', async () => {
  const { image } = generateSyntheticGarment({ seed: 6, backgroundColor: WHITE, garmentColor: BLUE });
  const bytes = encodePng(image);
  const a = runPipelineForImage(product(), 'ref.png', await decodeImageBytes(bytes), { fidelityHints: { knownFillColor: BLUE } });
  const b = runPipelineForImage(product(), 'ref.png', await decodeImageBytes(bytes), { fidelityHints: { knownFillColor: BLUE } });
  assert.equal(a.manifest.assetId, b.manifest.assetId);
});

test('end-to-end: two different variants of the same productRef never collide on assetId', async () => {
  const { image } = generateSyntheticGarment({ seed: 7, backgroundColor: WHITE, garmentColor: BLUE });
  const decoded1 = await decodeImageBytes(encodePng(image));
  const { image: image2 } = generateSyntheticGarment({ seed: 8, backgroundColor: WHITE, garmentColor: [10, 10, 10] });
  const decoded2 = await decodeImageBytes(encodePng(image2));
