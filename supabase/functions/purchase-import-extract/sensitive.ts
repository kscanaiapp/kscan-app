/**
 * Receipt & Purchase Intelligence V1 — server copy of the sensitive-text scrub core.
 *
 * BYTE-IDENTICAL between the BEGIN/END markers to
 * services/purchaseImport/purchaseImportSensitive.ts. Deno and the app bundle
 * cannot share a module, so the core is restated, and
 * __tests__/purchaseImportParity.test.js fails the moment the two copies differ.
 *
 * BLOCK-RPI-26: card fragments, contact details and account references are
 * removed from every free-text value before a response leaves this function.
 */
