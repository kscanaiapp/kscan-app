...
Deno.serve(async (req: Request) => {
  ...
  const entitlement = await resolveVtoEntitlement(authUser.id);
  ...
  const devScenariosAllowed = Deno.env.get('VTO_ALLOW_DEV_SCENARIOS') === 'true';
  ...
  const selection = resolveVtoProvider({
