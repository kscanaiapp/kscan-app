...
} from '../_shared/appleAuth/revocationGate.ts';
...
async function processClaimedRequest(
  ...
  // Sign in with Apple revocation (IOS29-NEW-003). Apple requires that deleting
  // an account created with Sign in with Apple also revokes that authorization
  // (TN3194, Guideline 5.1.1(v)).
  //
  // This must run BEFORE auth.admin.deleteUser: public.apple_auth_credentials is
  // keyed by auth.users(id) ON DELETE CASCADE, so once the auth user is gone the
  // stored token is gone with it and revocation becomes permanently impossible.
  //
  // It also runs before the AUTH_DELETE_STARTED ledger entry. If Apple blocks,
  // the auth delete genuinely never started, and recording that it had would
  // misstate the lifecycle.
  //
  // No Apple logic lives here. apple-revoke-credential remains the single
  // revocation authority; this worker only supplies its own trusted deletion
  // candidate and reads the answer.
  ...
  if (isBlockingRevocationStatus(appleRevocation.status)) {
    // Throwing routes into the existing failure path: schedule_deletion_retry_or_fail
    // leaves the request retryable with backoff, dead-letters it after attempts
    // are exhausted, and never marks it purged. Nothing has been irreversibly
    // removed at this point, so a later attempt can still revoke.
    logEvent('apple_revocation_blocked', {
      uid: shortUserId(userId),
      status: appleRevocation.status,
      detail: appleRevocation.detail ?? null,
    });
    throw new Error(`apple_revocation_blocked:${appleRevocation.status}`);
  }

  logEvent('apple_revocation_complete', {
    uid: shortUserId(userId),
    status: appleRevocation.status,
  });
  if (!(await heartbeat(requestId, workerId))) return { status: 'lost_lease' };
