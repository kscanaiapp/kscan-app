...
function harness(options: {
  ...
  const deps: Partial<HandlerDeps> = {
    ...
    rest: (path: string, init: RequestInit = {}) => {
      ...
      if (method === 'PATCH' && path.startsWith('profiles?')) {
        return Promise.resolve(
          options.profileOk === false ? jsonResponse({ error: 'nope' }, 500) : jsonResponse({}, 200),
        );
      }
    ...
    // Production security controls (RP-06A). These MUST be stubbed on every
    // harness: the real implementations open an admin Supabase client and hit
    // the network, so an un-stubbed test would not be hermetic.
    revokeSessions: (userId: string, accessToken?: string | null) => {
    ...
    banAuthUser: (userId: string, duration: string) => {
...
/**
 * RP-06A: the staging lineage merely REPORTED a legacy `pending` row. The
 * production lineage upgrades it in place, minting the grace window and
 * restoration token the retired pre-Build-29 intake never created -- without
 * which the holder can neither restore nor be purged correctly. Production
 * behaviour is the accepted one and is preserved here.
 */
Deno.test('a legacy pending row is UPGRADED into the restorable lifecycle', async () => {
  const { calls, handler } = harness({
    existing: [createdRow({ status: 'pending', grace_period_ends_at: null })],
  });
  const payload = await (await handler(postRequest())).json();

  assertEquals(payload.status, 'deactivated');
  assertEquals(payload.upgradedFromLegacy, true);
  assertEquals(payload.alreadyRequested, true);
  assertEquals(
    calls.filter((c) => c.kind === 'rest' && c.method === 'POST').length,
    0,
    'a pre-token pending row must be upgraded in place, never duplicated',
  );

  const upgrade = calls.find(
    (c) => c.kind === 'rest' && c.method === 'PATCH' && (c.body as Record<string, unknown>)?.status === 'deactivated',
  );
  assert(upgrade, 'the legacy row is patched to deactivated');
  const upgradeBody = upgrade?.body as Record<string, unknown>;
  assertNotEquals(upgradeBody.restoration_token_hash, undefined);
  assertNotEquals(upgradeBody.restoration_token_hash, RAW_TOKEN);
});

Deno.test('the legacy upgrade persists the token hash before emailing the raw token', async () => {
  const { calls, handler } = harness({
    existing: [createdRow({ status: 'pending', grace_period_ends_at: null })],
  });
  await handler(postRequest());

  const hashIndex = calls.findIndex(
    (c) => c.kind === 'rest' && (c.body as Record<string, unknown>)?.restoration_token_hash,
  );
  const emailIndex = calls.findIndex((c) => c.kind === 'email');
  assert(hashIndex !== -1 && emailIndex !== -1);
  assert(hashIndex < emailIndex, 'hash must be durable before the link is delivered');

  for (const call of calls.filter((c) => c.kind === 'rest')) {
    assert(
      !JSON.stringify(call.body ?? {}).includes(RAW_TOKEN),
      'raw token must never reach the database',
    );
  }
});

// ── Production security controls (RP-06A) ───────────────────────────────────

Deno.test('sessions are revoked with the caller token and the outcome is reported', async () => {
  const { calls, handler } = harness();
  const payload = await (await handler(postRequest())).json();

  const revoke = calls.find((c) => c.kind === 'revoke');
  assert(revoke, 'session revocation is invoked on acceptance');
  assertEquals((revoke?.body as Record<string, unknown>)?.userId, USER_ID);
  assertEquals(payload.sessionRevocationOk, true);
});

Deno.test('a failed revocation is reported truthfully but does not reject the deletion', async () => {
  ...
  assertEquals(response.status, 200);
  assertEquals(payload.status, 'deactivated');
  assertEquals(payload.sessionRevocationOk, false);
});

Deno.test('Auth is banned for exactly the 30-day grace window', async () => {
  const { calls, handler } = harness();
  await handler(postRequest());

  const ban = calls.find((c) => c.kind === 'ban');
  assert(ban, 'auth ban is applied on acceptance');
  assertEquals((ban?.body as Record<string, unknown>)?.duration, '720h');
});

Deno.test('a failed Auth ban does not reject an otherwise-accepted deletion', async () => {
  const { handler } = harness({ banOk: false });
  const response = await handler(postRequest());
  assertEquals(response.status, 200);
  assertEquals((await response.json()).status, 'deactivated');
});

Deno.test('a deletion whose profile cannot be deactivated is REJECTED and marked failed', async () => {
  const { calls, handler } = harness({ profileOk: false });
  const response = await handler(postRequest());

  assertEquals(response.status, 500);
  assertEquals((await response.json()).error, 'Unable to deactivate account');

  const failed = calls.find(
    (c) => c.kind === 'rest' && c.method === 'PATCH' && (c.body as Record<string, unknown>)?.status === 'failed',
  );
  assert(failed, 'compensating failed marker written');
  const failedBody = failed?.body as Record<string, unknown>;
  assertEquals(failedBody.failure_code, 'PROFILE_DEACTIVATION_FAILED');
  assertEquals(failedBody.restoration_token_hash, null);

  assertEquals(
    calls.filter((c) => c.kind === 'email').length,
    0,
    'a rejected deactivation never delivers a restoration link',
  );
});
