// Private Dressing Room ↔ Elise versioned contract — Edge Function mirror.
//
// This file MIRRORS mobile types/privateDressingRoomElise.ts, which is the
// governing source. Everything from the "Schema version" banner below is copied
// byte-for-byte; __tests__/privateDressingRoomEliseContract.test.js asserts the
// two stay identical and fails the build if they diverge.
//
// Same convention as reasoningContract.ts ↔ types/fashionReasoning.ts: the
// React Native bundle and the Deno function cannot import one shared module, so
// they share one governing source and a static parity gate instead.
//
// PURE MODULE: no Deno APIs, no imports.
